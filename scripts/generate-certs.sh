...
if test $# -ne 1
then
  echo "Usage: generate-certs.sh <password>"
  exit 1
fi
...
openssl req -new -x509 -days 365 -key $PATH_CA/ca.key -out $PATH_CA/ca.crt -passin pass:$PASSWORD -subj "/C=CA/ST=./L=./O=ACME Signing Authority Inc/CN=."
...
openssl req -new -key $PATH_SERVER/server.key -out $PATH_TMP/server.csr -passout pass:$PASSWORD -subj "/C=CA/ST=./L=./O=ACME Signing Authority Inc/CN=$FQDN"
...
openssl req -new -key $PATH_CLIENT/client.key -out $PATH_TMP/client.csr -passout pass:$PASSWORD -subj "/C=CA/ST=./L=./O=ACME Signing Authority Inc/CN=CLIENT"
